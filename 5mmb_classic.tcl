     ...
     if { $keyboard=="de" } {
       ...
       set oem "oem3"
 		  	...
 		  	if { [llength $line] != 2 } { puts "ERROR: incorrect number of elements line $line" ; puts "hit any key to return" ; gets stdin char ; return }
				...
				if { $monitor !=".5k" && $monitor !="1k" && $monitor !="2k" && $monitor !="3k" && $monitor !="4k" }  { puts "ERROR: monitor choices are .5k/1k/2k/3k/4k" ; return }
...
<Command LaunchHiresAndRename>
	<SendPC %1%>
	<Run "G:/World of Warcraft/_classic_/Wow.exe" -nosound>
	<WaitForInputIdle 400>
	<TargetWin "World of Warcraft">  
	<RenameTargetWin Unused%2%>  
	<WaitForInputIdle 400>
	...
	foreach mainraid $mainraids {
		...
		for { set i 0 } { $i<[array size $arrayname] } { incr i } {
			...
			#puts "passwd is $passwd"
	  		...
	  		puts $hK "  <LaunchAndRename $computer($cpunum) $winname $bnet_account $passwd [lindex $raidhash($windowcount($myraid)) [expr $windex($myraid)*4+0]] [lindex $raidhash($windowcount($myraid)) [expr $windex($myraid)*4+1]] [lindex $raidhash($windowcount($myraid)) [expr $windex($myraid)*4+2]] [lindex $raidhash($windowcount($myraid)) [expr $windex($myraid)*4+3]]>"
			incr windex($myraid)
		}
		foreach raid [array names windowcount] { 
			set windex($raid) 0
		...
		regexp {([a-z]|[A-Z])([0-9])?} $myraid match foo cpunum
	  	...
	  	set passwd [lindex $thistoon 2]
	  	set winname ${toonname}_${cpunum}$acct_winname($account)
	  	puts $hK "  <ResetWindowPosition $computer($cpunum) $winname $bnet_account $passwd [lindex $raidhash($windowcount($myraid)) [expr $windex($myraid)*4+0]] [lindex $raidhash($windowcount($myraid)) [expr $windex($myraid)*4+1]] [lindex $raidhash($windowcount($myraid)) [expr $windex($myraid)*4+2]] [lindex $raidhash($windowcount($myraid)) [expr $windex($myraid)*4+3]]>"
			incr windex($myraid)
		}
	}
...
//-----------------------------------------------------------
<Hotkey ScrollLockOn A-Z, 1-9, Shift, Ctrl, Alt, Plus, Minus, Esc , Tab, Divide, F1-F12 except 1-5,F9,E,F,Q,H, W, A, S, D, R, T, Y, I, U, J, V>}
	puts $hK $winlabels
	puts $hK { <Key %Trigger%>}
	puts $hK ""
	puts $hK {//-----------------------------------------------------------
...
<Hotkey ScrollLockOn Alt 1>}
	...
	puts $hK ""
	puts $hK {// Same magic for 2-5}
	puts $hK {<Hotkey ScrollLockOn 2-5>}
	puts $hK $winlabels
	puts $hK "\t<Key %Trigger%>"
	set totallabels 0
	for { set i 0 } { $i<[array size toons] } { incr i } {
...
if { ! $nosmoverwrite } { 
	...
	while { [gets $sM line] >= 0 } {
	  if { [regexp "^FSMB_tank" $line ] } {
	    ...
	    set first false
	    for { set i 0 } { $i<[array size toons] } { incr i } {
	      if { [string tolower [lindex $toons($i) 4]] == "tank" } {
